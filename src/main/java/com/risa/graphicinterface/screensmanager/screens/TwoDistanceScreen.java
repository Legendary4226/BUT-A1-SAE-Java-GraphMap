...
public class TwoDistanceScreen extends JPanel {
    ...
    private void buildPanel() {
        ...
        add(ComponentsGenerator.jLabel("Selectionner deux sites pour savoir si ils sont exactement a deux distance.", true, true, true));

        add(ComponentsGenerator.verticalGlue(20));

        add(ComponentsGenerator.jLabel("Site 1", true, true));
        add(ComponentsGenerator.customNodesComboBox(true, selectorOne));
        selectorOne.fill(screensManager.getGraphSAE().getNoeuds().values());
        add(ComponentsGenerator.jLabel("Site 2", true, true));
